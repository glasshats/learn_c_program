...
int main(){
    ...
    fp = fopen("file.txt", "r");
    if(fp == NULL){
        printf("error  : %s \n ",strerror(errno));//The C library function char *strerror(int errnum) searches an internal array for the error number errnum and returns a pointer to an error message string. The error strings produced by strerror depend on the developing platform and compiler.
                                //          ^
                                        //  |
